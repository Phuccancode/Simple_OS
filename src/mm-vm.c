...
void sorted_insert(struct vm_rg_struct *head, struct vm_rg_struct *new_rg)
{
  struct vm_rg_struct *current;
  if(head == NULL || (head)->rg_start >= new_rg->rg_start)
  {
    new_rg->rg_next = head;
    head = new_rg;
  }
  ...
  {
    current = head;
    while(current->rg_next != NULL && current->rg_next->rg_start < new_rg->rg_start)
    {
      current = current->rg_next;
    }
    new_rg->rg_next = current->rg_next;
    current->rg_next = new_rg;
  }
}
void sort(struct vm_rg_struct *head)
{
  struct vm_rg_struct *sorted = NULL;
  struct vm_rg_struct *current = head;
  struct vm_rg_struct *next;
...
void merge_adjacent_region(struct vm_rg_struct *head)
{
  struct vm_rg_struct *current = head;
  struct vm_rg_struct *next;
  ...
  {
    next = current->rg_next;
    if(current->rg_end == next->rg_start)
    {
      current->rg_end = next->rg_end;
...
int __free(struct pcb_t *caller, int vmaid, int rgid)
{
  ...
  for(int i=0 ; i<incnumpage;i++){
    tlb_clear_bit_valid(caller->tlb,caller->pid,pgn+i);
...
int validate_overlap_vm_area(struct pcb_t *caller, int vmaid, int vmastart, int vmaend)
{
  ...
  struct vm_area_struct *vmatocheck = get_vma_by_num(caller->mm, vmaid);
  if (vmatocheck == NULL) return -1;
  while (vma!=NULL) {
    if (vma != vmatocheck && OVERLAP(vmatocheck->vm_start, vmatocheck->vm_end, vma->vm_start, vma->vm_end)) {
      return -1;
    }
    vma = vma->vm_next;
  }
